...
void CudaPlatform::launch_kernel(DeviceId dev,
  ...
  // DEBUG is set 
    ...
    avg /= iterations;
    float std_dev = 0.0;
    for(auto &n: times) std_dev += ((n - avg) * (n - avg));
    if(iterations > 1){
        std_dev = sqrt(std_dev / (ITERATIONS - 1));
    }
    std::cout << "Kernel runtime " << avg << " Std dev: " << std_dev << "\n";
    }
    // if (runtime_->profiling_enabled()) {
...
std::string CudaPlatform::compile_cuda(DeviceId dev, const std::string& filename, const std::string& program_string) const {
    ...
    std::cout << "Cuda compile time " << std::chrono::duration_cast<std::chrono::milliseconds>(t2-t1).count() << std::endl;
