...
__global__ void match_naive_opt_spec_manual_nu(const int* __restrict__ d_input_string, int input_size, int n_hat, int num_blocks_minus1, int* d_match_result) {
...
void matchNaiveSpecManualOptNUWrapper(PFAC_handle_t handle, dim3 grid, dim3 block,const int* d_input_string, int input_size, int n_hat, int num_blocks_minus1, int* d_match_result){
    ...
    auto kernel_instance = program.kernel("match_naive_opt_spec_manual_nu_jit")
       .instantiate()
