...
#define ROW_BLOCK_DIM_X255 32
#define ROW_BLOCK_DIM_Y255 16
...
__global__ void rowConvolutionFilter255(
    ...
    __shared__ float sData[ROW_BLOCK_DIM_Y255][(ROW_RESULT_STEP + 2*ROW_HALO_STEP255) * ROW_BLOCK_DIM_X255];

    //offset to left halo edge
    const int baseX = (blockIdx.x * ROW_RESULT_STEP) * ROW_BLOCK_DIM_X255 - ROW_HALO_STEP255 * ROW_BLOCK_DIM_X255 + threadIdx.x;
...
#pragma unroll
    ...
    for (int i = ROW_HALO_STEP255; i < ROW_HALO_STEP255 + ROW_RESULT_STEP; i++) {
...
#pragma unroll
    ...
    for (int i = 0; i < ROW_HALO_STEP255; i++) {
...
#pragma unroll
    ...
    for (int i = ROW_HALO_STEP255 + ROW_RESULT_STEP; i < ROW_HALO_STEP255 + ROW_RESULT_STEP + ROW_HALO_STEP255; i++) {
...
#define COL_BLOCK_DIM_X255 16
#define COL_BLOCK_DIM_Y255 32
...
)
{
    ...
    __shared__ float sData[COL_BLOCK_DIM_X255][(COL_RESULT_STEP + 2 * COL_HALO_STEP255) * COL_BLOCK_DIM_Y255 + 1]; //+1 to avoid shared mem bank conflicts
    
    const int baseX = blockIdx.x * COL_BLOCK_DIM_X255 + threadIdx.x;
    const int baseY = blockIdx.y * COL_BLOCK_DIM_Y255 * COL_RESULT_STEP - COL_HALO_STEP255 * COL_BLOCK_DIM_Y255 + threadIdx.y;
...
#pragma unroll
    ...
    for (int i = COL_HALO_STEP255; i < COL_HALO_STEP255 + COL_RESULT_STEP; i++) {
...
#pragma unroll
    ...
    for (int i = 0; i < COL_HALO_STEP255; i ++) {
...
#pragma unroll
    ...
    for (int i = COL_HALO_STEP255 + COL_RESULT_STEP; i < COL_HALO_STEP255 + COL_RESULT_STEP + COL_HALO_STEP255; i++) {
