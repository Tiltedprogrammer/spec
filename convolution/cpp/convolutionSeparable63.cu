...
__global__ void rowConvolutionFilter63(
    ...
    __shared__ float sData[ROW_BLOCK_DIM_Y63][(ROW_RESULT_STEP + 2*ROW_HALO_STEP) * ROW_BLOCK_DIM_X63];

    //offset to left halo edge
    const int baseX = (blockIdx.x * ROW_RESULT_STEP) * ROW_BLOCK_DIM_X63 - ROW_HALO_STEP * ROW_BLOCK_DIM_X63 + threadIdx.x;
...
)
{
    ...
    __shared__ float sData[COL_BLOCK_DIM_X63][(COL_RESULT_STEP + 2 * COL_HALO_STEP) * COL_BLOCK_DIM_Y63 + 1]; //+1 to avoid shared mem bank conflicts
    
    const int baseX = blockIdx.x * COL_BLOCK_DIM_X63 + threadIdx.x;
    const int baseY = blockIdx.y * COL_BLOCK_DIM_Y63 * COL_RESULT_STEP - COL_HALO_STEP * COL_BLOCK_DIM_Y63 + threadIdx.y;
